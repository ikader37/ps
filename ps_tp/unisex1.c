...
int nombre_homme;
int nombre_femme;
int nombre_cap;//Definit la capacite totale de la douche
int nombre_in_place_vide;//Definit le nombre de place qui reste actuelle dans la douche a occupee
...
int genrechrch=2;//Pour controler le genre dans la bouche (1 pour femme et 0 pour homme)
...
void man_enter_in_bathroom(){
  ...
  if (nombre_in_place_vide==nombre_cap) {//Verifions que la douche est vide... il ny a personne
    /* code */

    printf("La douche est vide \n");
    ...
    //Dinumuons maintenant la capacite de la douche
    sem_wait(&semaCapacite);
    //Mettons a jour la capacite acctuelle de la douche
    sem_getvalue(&semaCapacite,&nombre_in_place_vide);
    printf("Homme THREAD %d Entre ...\n",id );
    printf("Capacite actuelle est %d...\n",nombre_in_place_vide );
...
void man_leave_bathroom(){
  ...
  //Augmenter la capaciter de la douche
  sem_post(&semaCapacite);
  //Mettr e a jour la variabble qui contient la capacite actuelle de la douche
	sem_getvalue(&semaCapacite, &nombre_in_place_vide);
	printf("\n La capacite de la douche est = %d\n \n", nombre_in_place_vide);

  //Verifions si cest la derniere personne a sortir
  if(nombre_in_place_vide==nombre_cap){
    pthread_mutex_lock(&mutex_semCherch);
    genrechrch=0;
    pthread_mutex_unlock(&mutex_semCherch);
...
void women_enter_in_bathroom(){
  ...
  int id=get_thread_id(pthread_self(),1);
    ...
    //Verifions si la douche est vide
    if(nombre_in_place_vide==nombre_cap){

      printf("La douche est vide . Sa capacité est : %d",nombre_in_place_vide);
    ...
    }else{//Sil ya deja quelqu'un dans la douche


      if (genrechrch==2) {//Si c'est une femme qui est entree dernierement alors
        /* code */
        pthread_mutex_lock(&mutex_semCherch);
        //Dinumuons maintenant la capacite de la douche
        sem_wait(&semaCapacite);
...
int main(int argc, char **argv) {
  ...
  int nb_wmen;
if (argc>2) {
  if (atoi(argv[1])>=1 && atoi(argv[2])>=1 && atoi(argv[3])) {
    /* code */

    nb_men=atoi(argv[1]);
    nb_wmen=atoi(argv[2]);
    nombre_cap=atoi(argv[2]);
  }else{
    ...
    do {
      ...
      scanf("%d",&nb_wmen );
      printf("Saisir nombre de personnes dans la douche:");
      scanf("%d",&nombre_cap);
    } while(nb_men<=0 && nb_wmen<=0 && nombre_cap<=0);
  }
}else{
  do {
    /* code */
  ...
  //  printf("OKKKK!!!\n");
    ...
    nombre_in_place_vide=nombre_cap;//Definir le nombre der place vide dans la douche

    //Initialisons le semaphore de la capacite de la douche
    sem_init(&semaCapacite, 0, nombre_cap);
